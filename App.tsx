...
import { 
  Transaction, ShoppingItem, FamilyMember, MandatoryExpense, Category, LearnedRule, WidgetConfig, AppNotification, FamilyEvent
} from './types';
...
export default function App() {
  ...
  const renderWidget = (id: string) => {
      switch (id) {
          case 'balance':
              return <SmartHeader balance={totalBalance} spent={currentMonthSpent} savingsRate={savingsRate} settings={settings} budgetMode={budgetMode} transactions={filteredTransactions} onToggleBudgetMode={() => setBudgetMode(prev => prev === 'family' ? 'personal' : 'family')} onTogglePrivacy={() => updateSettings({ ...settings, privacyMode: !settings.privacyMode })} onAddTransaction={() => setIsAddModalOpen(true)} onOpenAIChat={() => setIsAIChatOpen(true)} className="shrink-0" />;
          case 'month_chart':
              return (
  ...
  return (
    <div className="h-[100dvh] w-full bg-[#EBEFF5] dark:bg-black text-[#1C1C1E] dark:text-white flex flex-col md:flex-row font-sans overflow-hidden">
      ...
      <main ref={mainRef} className="flex-1 h-full p-4 md:p-8 pb-32 md:pb-8 relative md:ml-28 overflow-y-auto no-scrollbar md:overflow-hidden">
        <div className="w-full flex flex-col gap-4 min-h-full lg:h-full">
            <AnimatePresence mode="wait">
            {activeTab === 'overview' && (
                <motion.div key="overview" initial="initial" animate="in" exit="out" variants={pageVariants} className="flex flex-col gap-4 min-h-full lg:h-full">
            ...
            {activeTab === 'budget' && (
                <motion.div key="budget" initial="initial" animate="in" exit="out" variants={pageVariants} className="flex flex-col gap-6 min-h-full lg:h-full lg:overflow-y-auto no-scrollbar">
                    ...
                    <div className="flex flex-col xl:flex-row gap-6 items-stretch">
                        ...
                        <div className="w-full xl:w-1/3 flex flex-col gap-6 shrink-0 h-auto">
                            ...
                            <MandatoryExpensesList 
                                expenses={filteredMandatoryExpenses} 
            ...
            {activeTab === 'plans' && <motion.div key="plans" initial="initial" animate="in" exit="out" variants={pageVariants} className="min-h-full lg:h-full lg:overflow-y-auto no-scrollbar"><FamilyPlans events={events} setEvents={setEvents} settings={settings} members={members} onSendToTelegram={handleSendEventToTelegram} onDeleteEvent={handleDeleteEvent} /></motion.div>}
            {activeTab === 'shopping' && <motion.div key="shopping" initial="initial" animate="in" exit="out" variants={pageVariants} className="min-h-full lg:h-full lg:overflow-y-auto no-scrollbar"><ShoppingList items={shoppingItems} setItems={setShoppingItems} settings={settings} members={members} onSendToTelegram={handleSendShoppingToTelegram} /></motion.div>}
            {activeTab === 'services' && <motion.div key="services" initial="initial" animate="in" exit="out" variants={pageVariants} className="h-auto lg:h-full lg:overflow-y-auto no-scrollbar"><ServicesHub initialService={targetService} onClearService={() => setTargetService(null)} /></motion.div>}
            </AnimatePresence>
        </div>
      </main>
             ...
             {TAB_CONFIG.map(tab => {
                 ...
                 return (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)} className="relative flex flex-col items-center justify-center w-12 h-12 md:w-20 md:h-auto md:py-4 group">
                        ...
                        <span className={`relative z-10 ${isActive ? 'text-white md:text-blue-600' : 'text-gray-400 hover:text-gray-200 md:hover:text-gray-600'}`}>{React.createElement(tab.icon, { size: 24, strokeWidth: isActive ? 2.5 : 2 })}</span>
