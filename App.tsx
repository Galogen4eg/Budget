            ...
            <motion.div key="overview" initial="initial" animate="in" exit="out" variants={pageVariants} transition={pageTransition} className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 auto-rows-[180px] grid-flow-row-dense">
                    {settings.widgets.filter(w => w.isVisible).map(widget => {
                        switch(widget.id) {
                            case 'month_chart': if (filteredTransactions.length === 0) return null; return <div key={widget.id} className={`${widgetLayout.chart.col} ${widgetLayout.chart.row}`}><MonthlyAnalyticsWidget transactions={filteredTransactions} currentMonth={currentMonth} settings={settings} /></div>;
                            case 'category_analysis': if (filteredTransactions.length === 0) return null; return <div key={widget.id} className={`${widgetLayout.cat.col} ${widgetLayout.cat.row}`}><CategoryAnalysisWidget transactions={filteredTransactions} categories={categories} settings={settings} onClick={() => setActiveTab('budget')} /></div>;
                            case 'goals': if (goals.length === 0) return null; return <div key={widget.id} className={`${widgetLayout.goal.col} ${widgetLayout.goal.row}`}><GoalsSection goals={goals} settings={settings} onEditGoal={(g) => { setEditingGoal(g); setIsGoalModalOpen(true); }} onAddGoal={() => { setEditingGoal(null); setIsGoalModalOpen(true); }} /></div>;
                            case 'recent_transactions': if (filteredTransactions.length === 0) return null; return <div key={widget.id} className={`${widgetLayout.hist.col} ${widgetLayout.hist.row}`}><RecentTransactionsWidget transactions={filteredTransactions} categories={categories} members={members} settings={settings} onTransactionClick={handleEditTransaction} onViewAllClick={() => setActiveTab('budget')} /></div>;
