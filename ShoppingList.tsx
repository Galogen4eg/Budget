...
import { Plus, Trash2, Check, Share, X, ScanLine, ShoppingBag, Loader2, Play, Edit2, ChevronLeft, Mic, BrainCircuit, ArrowRight, Send } from 'lucide-react';
...
const ShoppingListMobile: React.FC<ShoppingListProps> = ({ 
  ...
  // Quick Add State (Mobile)
  const [mobileQuickText, setMobileQuickText] = useState('');
  const [isSubmittingMobileQuick, setIsSubmittingMobileQuick] = useState(false);
  const liveMobileParsed = useMemo(() => {
    if (!mobileQuickText.trim()) return [];
    return parseQuickShoppingInput(mobileQuickText);
  }, [mobileQuickText]);
  ...
  const handleMobileQuickAdd = async (e: React.FormEvent) => {
    e.preventDefault();
    const raw = mobileQuickText.trim();
    if (!raw || isSubmittingMobileQuick) return;

    try {
      setIsSubmittingMobileQuick(true);
      const newItems = createShoppingItemsFromQuickText(raw, user?.uid || 'user');
      ...
      setIsSubmittingMobileQuick(false);
    }
  };
  ...
  const handleSaveItem = async () => {
      ...
      if (editingItem) {
          ...
          recordPurchaseEvent(newItem.title, 'added', { category: selectedAisle, amount: newItemAmount, unit: newItemUnit });
  ...
  const handleToggle = async (item: ShoppingItem) => {
      ...
      recordPurchaseEvent(item.title, updated.completed ? 'completed' : 'restored', {
  ...
  return (
    <div className="space-y-4 h-full flex flex-col relative">
        ...
        {/* Smart Quick Add Bar (Mobile) */}
        <div className="bg-white dark:bg-[#1C1C1E] p-3 rounded-2xl shadow-xs border border-surface-border dark:border-white/5">
          <form onSubmit={handleMobileQuickAdd} className="flex flex-col gap-2">
            <div className="relative flex items-center">
              <input 
                type="text"
                value={mobileQuickText}
                onChange={(e) => setMobileQuickText(e.target.value)}
                placeholder="Быстро: чипсы 2 шт, молоко 1 л..."
              ...
              <button 
                type="submit"
                disabled={!mobileQuickText.trim() || isSubmittingMobileQuick}
                className="absolute right-1.5 p-1.5 bg-[#4A7C59] hover:bg-[#3D694B] text-white rounded-lg transition disabled:opacity-40 cursor-pointer shadow-xs active:scale-95"
                title="Добавить"
              >
                <Plus size={14} strokeWidth={2.5} />
              </button>
            </div>
            {liveMobileParsed.length > 0 && (
              <div className="flex items-center gap-1.5 flex-wrap px-1 text-[11px] text-[#4A7C59] dark:text-green-400 font-semibold animate-in fade-in">
                <span className="w-1.5 h-1.5 rounded-full bg-[#4A7C59] animate-pulse" />
                {liveMobileParsed.map((p, idx) => (
                  <span key={idx} className="bg-[#EDF4EF] dark:bg-[#203425] px-2 py-0.5 rounded-md text-[11px]">
        ...
        <div className="flex gap-2 shrink-0 items-stretch">
            ...
            <button 
                ...
                className="bg-[#0088CC] hover:bg-[#0077B5] text-white p-4 rounded-[2rem] shadow-lg active:scale-95 transition-transform disabled:opacity-50 disabled:shadow-none flex items-center justify-center cursor-pointer"
                title="Отправить список покупок в Telegram"
            >
                <Send size={20} />
            </button>
        </div>
                        ...
                        {items.filter(i => i.completed).map(item => (
                            <div key={item.id} className="flex items-center justify-between p-3 bg-gray-100 dark:bg-[#2C2C2E] rounded-2xl">
                                ...
                                <div className="flex items-center gap-2">
                                    <button onClick={() => onMoveToPantry(item)} className="p-1.5 bg-blue-100 dark:bg-blue-900/30 text-blue-500 rounded-lg" title="В кладовку"><ArrowRight size={14}/></button>
