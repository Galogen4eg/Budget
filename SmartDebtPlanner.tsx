...
import { 
  ...
  HelpCircle, Settings2, RefreshCw, Plus, X, Lock, Calculator, CalendarClock, CreditCard, Trash2, Edit2, ChevronDown, ChevronUp, BarChart3
} from 'lucide-react';
...
const SmartDebtPlanner: React.FC<Props> = ({ transactions, debts, settings }) => {
  ...
  const simulation = useMemo(() => {
      ...
      // Generate Chart Data points for the next 6 months
      let currentBal = activeDebts.reduce((s,d) => s + (d.currentBalance || 0), 0);
      ...
      for (let m = 0; m <= 6; m++) {
          ...
          if (m > 0) {
              const payments = schedule.filter(s => s.date >= mDate && s.date < nextMonth);
              const totalPaid = payments.reduce((sum, p) => sum + p.amount, 0);
              currentBal = Math.max(0, currentBal - totalPaid);
              chartData.push({
                  month: mNames[mDate.getMonth()],
                  balance: currentBal,
                  paid: totalPaid
              });
          } else {
              // Now
              const remainingThisMonthPayments = schedule.filter(s => s.date >= today && s.date < nextMonth);
              const totalPaidThisMonth = remainingThisMonthPayments.reduce((sum, p) => sum + p.amount, 0);
              currentBal = Math.max(0, currentBal - totalPaidThisMonth);
              chartData.push({
                  month: mNames[mDate.getMonth()],
                  balance: currentBal,
      ...
      return { schedule, totalRequired, totalCovered, criticalDate, chartData };
            ...
            <div className="space-y-4">
                ...
                <div className="bg-white dark:bg-[#1C1C1E] p-5 rounded-[2rem] border border-white dark:border-white/5 shadow-sm">
                    <div className="flex items-center gap-2 mb-4 text-gray-400">
                        <BarChart3 size={16} />
                        <span className="text-[10px] font-black uppercase tracking-widest">Прогноз остатка</span>
                    </div>
                    <div className="h-40 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={simulation.chartData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                                <defs>
                                ...
                                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#9ca3af' }} tickFormatter={(val) => `${(val / 1000).toFixed(0)}k`} />
                                <RechartsTooltip 
                                    content={({ active, payload }) => {
                                        if (active && payload && payload.length) {
                                            const data = payload[0].payload;
                                            return (
                                                <div className="bg-[#1C1C1E] dark:bg-[#2C2C2E] p-3 rounded-xl border border-white/10 shadow-xl">
                                                    <div className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">{data.month}</div>
                                                    <div className="text-sm font-black text-white">
                                                        Остаток: {data.balance.toLocaleString()} ₽
                                                    </div>
                                                    <div className="text-xs font-bold text-indigo-400 mt-1">
                                                        Выплата: {data.paid.toLocaleString()} ₽
                                                    </div>
                                                </div>
                                            );
                                        }
                ...
                <div className="space-y-3 pt-2">
                    <button 
                        ...
                        className="w-full flex items-center justify-between p-4 bg-white dark:bg-[#1C1C1E] rounded-[2rem] border border-white dark:border-white/5 shadow-sm active:scale-95 transition-all"
                    >
                        <h3 className="text-sm font-black text-gray-800 dark:text-gray-200 uppercase tracking-widest flex items-center gap-2">
                            <Calendar size={16} className="text-indigo-500" /> Подробный график
                        </h3>
                        {isScheduleOpen ? <ChevronUp size={20} className="text-gray-400"/> : <ChevronDown size={20} className="text-gray-400" />}
                    </button>
                    ...
                    <AnimatePresence>
                        {isScheduleOpen && (
                            ...
                            >
                                {simulation.schedule.map((event, idx) => (
