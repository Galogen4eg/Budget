        ...
        const formatDate = (dateStr) => { if(!dateStr) return ''; const options = { month: 'short', day: 'numeric', year: 'numeric' }; return new Date(dateStr).toLocaleDateString('ru-RU', options); };
        ...
        const Import = {
            ...
            process(rows) {
                ...
                for(let i = headerRowIdx + 1; i < rows.length; i++) {
                    ...
                    const cat = this.categorize(desc);
            ...
            categorize(desc) {
                ...
                if(d.includes('пятерочка') || d.includes('перекресток') || d.includes('магнит') || d.includes('ашан') || d.includes('лента')) return 'Продукты';
                if(d.includes('яндекс') || d.includes('uber') || d.includes('лукойл') || d.includes('газпром') || d.includes('такси')) return 'Транспорт';
                if(d.includes('аптека') || d.includes('здоровье') || d.includes('доктор')) return 'Здоровье';
                if(d.includes('кафе') || d.includes('ресторан') || d.includes('mcdonalds') || d.includes('бургер') || d.includes('kfc')) return 'Кафе';
                if(d.includes('кино') || d.includes('ivi') || d.includes('netflix') || d.includes('театр')) return 'Развлечения';
                if(d.includes('жкх') || d.includes('свет') || d.includes('газ') || d.includes('вода')) return 'Дом';
                if(d.includes('зарплата') || d.includes('аванс') || d.includes('выплата')) return 'Зарплата';
                return 'Разное';
            },
        ...
        const Dashboard = {
            ...
            getWidgetIcon(type) {
                const icons = {
                    'budget_overview': '<span class="bg-blue-100 dark:bg-blue-900/30 text-blue-600 p-1 rounded-md">💰</span>',
                    'today_schedule': '<span class="bg-red-100 dark:bg-red-900/30 text-red-600 p-1 rounded-md">📅</span>',
                    'shopping_summary': '<span class="bg-orange-100 dark:bg-orange-900/30 text-orange-600 p-1 rounded-md">🛒</span>',
                    'quick_actions': '<span class="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 p-1 rounded-md">⚡</span>',
                    'recent_transactions': '<span class="bg-green-100 dark:bg-green-900/30 text-green-600 p-1 rounded-md">💳</span>'
                };
                return icons[type] || '';
            },
                ...
                return `
                    <div class="grid grid-cols-2 gap-3">
                        <button onclick="Budget.openModal()" class="flex flex-col items-center justify-center p-3 bg-gray-50 dark:bg-white/5 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-xl transition-colors group">
                            <div class="w-8 h-8 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center mb-1 group-hover:scale-110 transition-transform">💰</div>
                        ...
                        <button onclick="Calendar.openAddModal()" class="flex flex-col items-center justify-center p-3 bg-gray-50 dark:bg-white/5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors group">
                            <div class="w-8 h-8 rounded-full bg-red-100 text-red-600 flex items-center justify-center mb-1 group-hover:scale-110 transition-transform">📅</div>
                        ...
                        <button onclick="Shopping.openAddModal()" class="flex flex-col items-center justify-center p-3 bg-gray-50 dark:bg-white/5 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-xl transition-colors group">
                            <div class="w-8 h-8 rounded-full bg-green-100 text-green-600 flex items-center justify-center mb-1 group-hover:scale-110 transition-transform">🛒</div>
                        ...
                        <button onclick="Shopping.openScanner()" class="flex flex-col items-center justify-center p-3 bg-gray-50 dark:bg-white/5 hover:bg-purple-50 dark:hover:bg-purple-900/20 rounded-xl transition-colors group">
                            <div class="w-8 h-8 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center mb-1 group-hover:scale-110 transition-transform">📷</div>
                    ...
                    <div class="mb-8 h-full flex flex-col">
                        ...
                        <div class="card p-8 flex-1 overflow-hidden flex flex-col">
                            ...
                            <div class="space-y-2 overflow-y-auto pr-2">
                                ...
                                listTxs.map(t => `
                                    <div class="flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-white/5 rounded-2xl transition-all group">
                                        <div class="flex items-center gap-4">
                                            <div class="w-10 h-10 rounded-full flex items-center justify-center text-lg shadow-sm ${t.type === 'income' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}">
                                                ${t.type === 'income' ? '↓' : '↑'}
                                            </div>
                                            <div>
                                                <p class="font-semibold text-gray-900 dark:text-white">${t.description}</p>
            ...
            render() {
                const header = `
                    <div class="flex justify-between items-center mb-6">
                        ...
                        <div class="flex gap-2">
                            <button onclick="Calendar.openTemplates()" class="p-2 rounded-lg bg-yellow-100 text-yellow-600 hover:bg-yellow-200">⭐</button>
                    ...
                    <div>
                        ...
                        <div class="space-y-4">
                            <div class="p-4 bg-gray-50 dark:bg-white/5 rounded-xl">
                                ...
                                ${!s.telegram.chat_id ? `
                                    <input type="text" placeholder="Bot Token" class="ios-input mb-2 text-sm" id="tg-token">
                                    <p class="text-[10px] text-gray-400">Введите токен от @BotFather</p>
                                ` : ''}
                            </div>
            ...
            connectTelegram() {
                ...
                if(s.telegram.chat_id) {
                    ...
                    if(token) {
                        ...
                        s.telegram.chat_id = '123456'; // Simulated ID
                        DB.save('settings', s);
                        alert('Telegram подключен (симуляция)');
                        router.refresh();
                    } else {
                        alert('Введите токен');
                    }
