        ...
        <AnimatePresence initial={false} mode="popLayout">
            ...
            {currentView === 'categories' && (
                ...
                >
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar pb-safe">
                        {
                            categories.filter(c => !c.parentId).sort((a, b) => a.label.localeCompare(b.label)).map(parentCat => {
                                ...
                                return (
                                    <div key={parentCat.id} className="bg-white dark:bg-[#1C1C1E] rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-white/5">
                                        {family.map((cat, idx) => {
                                            ...
                                            return (
                                                <button
                                                    ...
                                                    className={`w-full flex items-center justify-between px-4 py-3.5 active:bg-gray-50 dark:active:bg-[#2C2C2E] transition-colors ${idx !== family.length - 1 ? 'border-b border-gray-100 dark:border-white/5' : ''} ${isChild ? 'pl-8 bg-gray-50/50 dark:bg-[#1C1C1E]/50' : ''}`}
                                                >
                                                    <div className="flex items-center gap-3 overflow-hidden">
                                                        <div className={`rounded-full flex items-center justify-center text-white ${isChild ? 'w-6 h-6' : 'w-8 h-8'}`} style={{ backgroundColor: cat.color }}>
                                                            {getIconById(cat.icon, isChild ? 12 : 16)}
                                                        </div>
                                                        <span className={`text-[16px] truncate ${categoryId === cat.id ? 'text-[#007AFF] font-medium' : 'text-black dark:text-white'} ${isChild ? 'text-sm' : ''}`}>{cat.label}</span>
                                                    </div>
