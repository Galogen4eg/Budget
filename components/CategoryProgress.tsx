...
const CategoryProgress: React.FC<CategoryProgressProps> = ({ transactions, settings, categories, onCategoryClick, onSubCategoryClick, currentMonth, selectedDate }) => {
  ...
  const categoryData = parentCategories.map(parentCat => {
    ...
    const merchants = familyTransactions.reduce((acc, t) => {
      // If the transaction has a child category, we can use its label, else we use note or parent label
  ...
  return (
    <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2.5rem] border border-white dark:border-white/5 shadow-soft dark:shadow-none space-y-4 transition-all w-full h-full flex flex-col overflow-hidden">
