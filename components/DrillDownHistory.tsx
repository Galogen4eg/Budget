...
const DrillDownHistory: React.FC<DrillDownHistoryProps> = ({ 
  ...
  const searchedTransactions = useMemo(() => {
    ...
    if (selectedCategoryId && selectedCategoryId !== 'all') {
        const childrenIds = categories.filter(c => c.parentId === selectedCategoryId).map(c => c.id);
        const validIds = [selectedCategoryId, ...childrenIds];
        result = result.filter(tx => validIds.includes(tx.category));
    }
