...
import { 
  ...
  LayoutGrid, List, History, Sparkles, Send, Hourglass, Users2, Filter
} from 'lucide-react';
                ...
                <div className="flex items-center gap-3 shrink-0">
                    ...
                    <div className="flex items-center bg-gray-100 dark:bg-white/5 p-1 rounded-xl">
                        <button 
                            ...
                            className={`p-2 rounded-lg transition-all ${filterMemberId === 'all' ? 'bg-white dark:bg-[#2C2C2E] shadow-sm text-black dark:text-white' : 'text-gray-400 hover:bg-gray-200/50 dark:hover:bg-white/10'}`}
                            title="Все участники"
                        >
                            <Users2 size={16} />
                        </button>
                        {members.map(m => (
                            <button 
                                ...
                                className={`p-1.5 rounded-lg transition-all ${filterMemberId === m.id ? 'bg-white dark:bg-[#2C2C2E] shadow-sm ring-1 ring-black/5 dark:ring-white/10' : 'opacity-60 hover:opacity-100'}`}
                                title={m.name}
                            >
                    ...
                    <div className="w-px h-6 bg-gray-300 dark:bg-white/10 mx-0.5"></div>
                    ...
                    {viewMode === 'calendar' && (
                        <div className="flex items-center bg-black/5 dark:bg-white/5 rounded-xl p-1">
                            ...
                            <button onClick={() => {
                                ...
                                setCurrentDate(now);
                                setSelectedDate(now);
                            }} className="px-3 py-1.5 text-xs font-bold hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                                Сегодня
                            </button>
                            <button onClick={() => changeDate(1)} className="p-2 hover:bg-white dark:hover:bg-white/10 hover:shadow-sm rounded-lg transition-all active:scale-90 text-gray-600 dark:text-gray-300">
                    ...
                    <button 
                        ...
                        className="bg-blue-500 hover:bg-blue-600 active:scale-95 text-white h-11 w-11 shrink-0 rounded-xl flex items-center justify-center transition-all shadow-lg shadow-blue-500/25"
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-7 grid-rows-6 flex-1 gap-px bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-3xl overflow-hidden shadow-2xl shadow-black/5 min-h-0">
                            {calendarData.map((d, i) => {
                                ...
                                // Fix: Explicitly type dayEvents to resolve TypeScript unknown type issue on line 291
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="mt-1 space-y-1 overflow-hidden">
                                            {/* Fix: Explicitly cast sliced array to FamilyEvent[] to resolve Property 'map' does not exist on type 'unknown' error */}
                                            {(dayEvents.slice(0, 2) as FamilyEvent[]).map((event: FamilyEvent) => {
                                                const dimmed = isEventDimmed(event);
                                                return (
                                                    <div 
                                                        key={event.id} 
                                                        className={`text-[9px] py-0.5 px-1.5 rounded-md font-bold truncate shadow-sm transition-all ${dimmed ? 'bg-gray-200 dark:bg-white/10 text-gray-400 dark:text-gray-500' : 'text-white bg-blue-500'}`}
                                                    >
                                        ...
                                        {monthEvents.map(event => {
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    className={`group flex items-center p-4 rounded-3xl border border-transparent shadow-sm hover:shadow-lg transition-all cursor-pointer ${dimmed ? 'bg-gray-100/50 dark:bg-white/5 opacity-60 grayscale' : 'bg-white dark:bg-[#1C1C1E] hover:border-blue-500/20'}`}
                                                >
                                                    {/* Date Badge */}
                                                    <div className={`flex flex-col items-center justify-center w-14 h-14 rounded-2xl mr-6 border ${dimmed ? 'bg-gray-200/50 dark:bg-white/5 border-transparent text-gray-400' : 'bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/5 text-[#1C1C1E] dark:text-white'}`}>
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-3 mb-1">
                                                            <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-lg ${dimmed ? 'bg-gray-200 dark:bg-white/10 text-gray-500' : 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400'}`}>
                                                                <Clock size={12} className={dimmed ? 'text-gray-500' : 'text-blue-600 dark:text-blue-400'}/>
                                                        ...
                                                        <h4 className={`text-lg font-bold truncate pr-4 ${dimmed ? 'text-gray-500' : 'text-[#1C1C1E] dark:text-white'}`}>{event.title}</h4>
                                                        {attendeeNames && (
                    ...
                    {selectedDayEvents.length > 0 ? (
                        <div className="space-y-8">
                            {selectedDayEvents.map(event => {
                                ...
                                const checklist: ChecklistItem[] = event.checklist || [];
                                const hasChecklist = checklist.length > 0;
                                const hasReminders = event.reminders && event.reminders.length > 0;
                                const dimmed = isEventDimmed(event);

                                return (
                                    <div key={event.id} className={`group animate-in fade-in slide-in-from-right-4 duration-400 cursor-pointer ${dimmed ? 'opacity-40 grayscale' : ''}`} onClick={() => onOpenEvent(event)}>
                                        <div className="flex items-center gap-4 mb-4">
                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${dimmed ? 'bg-gray-100 dark:bg-white/10 text-gray-400' : 'bg-blue-500/10 text-blue-500'}`}>
                                        ...
                                        <div className="space-y-4 ml-2 border-l-2 border-black/[0.03] dark:border-white/[0.05] pl-6 py-2">
                                            ...
                                            {hasChecklist && (
                                                <div className="space-y-2">
                                                    <div className="text-[9px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                                        <CheckSquare size={12} /> Чек-лист ({checklist.filter(i=>i.completed).length}/{checklist.length})
                                                    </div>
                                                    <div className="space-y-1">
                                                        ...
                                                        {checklist.length > 3 && (
                                                            <span className="text-[9px] text-gray-400 pl-5">+ еще {checklist.length - 3}</span>
                                                        )}
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {hasReminders && (
                                                <div className="flex flex-wrap gap-2">
                                                    {/* Fix: Explicitly cast reminders to number[] to ensure .map is recognized */}
                                                    {(event.reminders as number[]).map((r, i) => (
