...
import { Mail, Lock, User, Key, Eye, EyeOff, ShieldCheck, ArrowRight, UserPlus, LogIn, Users } from 'lucide-react';
...
export const LoginScreen: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) {
      toast.error('Пожалуйста, заполните почту и пароль');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#f7f4ee] bg-[radial-gradient(at_15%_15%,rgba(226,235,224,0.75)_0px,transparent_55%),radial-gradient(at_85%_15%,rgba(243,235,222,0.8)_0px,transparent_50%),radial-gradient(at_50%_85%,rgba(235,240,233,0.7)_0px,transparent_65%),radial-gradient(at_85%_85%,rgba(238,230,218,0.6)_0px,transparent_50%)] flex items-center justify-center p-4 sm:p-6 lg:p-8 font-sans text-[#1c241f] selection:bg-[#c8e8d0] selection:text-[#002110]">
      {/* Main Container */}
      <main className="w-full max-w-[450px] mx-auto">
        <motion.div 
        ...
        >
          ...
          {/* Mode Switcher Tabs */}
          <div className="mt-6 p-1 bg-[#ece7df] rounded-xl flex items-center text-xs font-semibold text-[#6b776d] relative">
            <button
              type="button"
              onClick={() => setActiveTab('login')}
              className={`flex-1 py-2 rounded-lg text-center transition-all duration-200 cursor-pointer ${
                activeTab === 'login'
                  ? 'bg-white text-[#1c241f] shadow-sm font-bold'
          ...
          {/* Registration Sub-Type (New Family vs Join Existing) */}
          <AnimatePresence mode="wait">
            {activeTab === 'register' && (
              <motion.div
                ...
                className="mt-3.5 pt-1 overflow-hidden"
              >
                <div className="bg-[#f0ece3] p-1 rounded-xl flex items-center gap-1 text-[11px] font-bold">
                  <button
                    type="button"
                    onClick={() => setRegisterType('new')}
                    className={`flex-1 py-1.5 rounded-lg flex items-center justify-center gap-1.5 transition-all ${
                      registerType === 'new'
                        ? 'bg-[#3e6b48] text-white shadow-2xs'
                        : 'text-[#525e55] hover:text-[#1c241f]'
                    }`}
                  >
                    <UserPlus size={13} />
                    Новый бюджет
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-3.5 relative z-10">
            ...
            {activeTab === 'register' && (
              ...
              >
                <label className="block text-xs font-bold text-[#354037] ml-1">Как к вам обращаться</label>
            ...
            {/* Invite / Family ID Field (Join Mode) */}
            {activeTab === 'register' && registerType === 'join' && (
              <motion.div
                initial={{ opacity: 0, y: -5 }}
                animate={{ opacity: 1, y: 0 }}
                className="space-y-1.5"
              >
                <label className="block text-xs font-bold text-[#354037] ml-1 flex items-center gap-1">
                  <Key size={13} className="text-[#3e6b48]" />
                  ID Семьи или Код Приглашения
                </label>
                <div className="relative">
                  <input
                    type="text"
                    ...
                    placeholder="Введите Family ID"
                    className="w-full px-4 py-2.5 bg-[#f5f2eb] border border-[#e4ded3] rounded-xl text-sm placeholder-[#9ba59d] text-[#1c241f] font-mono focus:bg-white focus:border-[#3e6b48] focus:ring-3 focus:ring-[#3e6b48]/15 focus:outline-none transition-all"
                  />
                </div>
              </motion.div>
            )}
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between ml-1">
                ...
                {activeTab === 'login' && (
                  <button
                    ...
                    className="text-xs font-semibold text-[#4a7c59] hover:text-[#33593c] hover:underline transition-colors"
                  >
                    Забыли пароль?
                  </button>
                )}
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-[#78857a] hover:text-[#1c241f] focus:outline-none transition-colors"
                >
            ...
            >
              <span>{isLoading ? 'Загрузка...' : activeTab === 'login' ? 'Войти' : 'Зарегистрироваться'}</span>
              <ArrowRight size={16} />
            </button>
          </form>
          ...
          <div className="mt-5">
            ...
            <div className="space-y-2.5">
              ...
              <button
                type="button"
                onClick={() => loginWithGoogle(activeTab === 'register' && registerType === 'join' ? inviteCode : undefined)}
                className="w-full py-2.5 px-4 bg-white hover:bg-[#f5f1ea] border border-[#ded7cb] text-[#2c332e] font-semibold rounded-xl shadow-xs transition-all duration-200 flex items-center justify-center gap-2.5 text-sm active:scale-[0.99] cursor-pointer"
              >
              ...
              {/* Secondary Switcher Action */}
              <button
                type="button"
                onClick={() => setActiveTab(activeTab === 'login' ? 'register' : 'login')}
                className="w-full py-2.5 px-4 bg-[#28362b] hover:bg-[#1e2a21] text-white font-semibold rounded-xl transition-all duration-200 flex items-center justify-center gap-2 text-sm shadow-xs active:scale-[0.99] cursor-pointer"
              >
                {activeTab === 'login' ? (
                  <>
                    <UserPlus size={16} />
