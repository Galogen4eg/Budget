...
export const LoyaltyCardMobileModal: React.FC<LoyaltyCardMobileModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[2500] bg-black/60 backdrop-blur-xs flex items-center justify-center p-0 sm:p-4 md:p-6 overflow-y-auto no-scrollbar font-body select-none">
      ...
      {/* Main Modal Container: Fullscreen on mobile, compact card container on PC */}
      <div className="w-full h-full sm:h-auto sm:max-w-md md:max-w-lg bg-[#FAF6F0] dark:bg-[#121214] text-[#2E3230] dark:text-gray-100 flex flex-col sm:rounded-3xl sm:shadow-2xl sm:border sm:border-[#E4E0D8] sm:dark:border-white/10 sm:overflow-hidden relative max-h-screen sm:max-h-[92vh] overflow-y-auto no-scrollbar transition-all">
        
        {/* 1. Header */}
        <header className="sticky top-0 left-0 right-0 z-50 pt-[env(safe-area-inset-top,0px)] bg-[#FAF6F0]/90 dark:bg-[#121214]/90 backdrop-blur-xl border-b border-[#E4E0D8]/60 dark:border-white/10 shrink-0">
          <div className="h-14 sm:h-16 px-4 flex items-center justify-between">
            <button 
              type="button"
              onClick={onClose}
              aria-label="Закрыть модальное окно"
              className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center rounded-full text-[#4A4E4A] dark:text-stone-300 hover:text-[#2E3230] hover:bg-[#F0ECE4] dark:hover:bg-white/10 transition-colors cursor-pointer"
            >
              <X size={22} />
            </button>
            
            <h1 className="text-base sm:text-lg font-headline font-semibold tracking-tight text-[#2E3230] dark:text-white text-center truncate px-2">
              Карта лояльности
            </h1>
