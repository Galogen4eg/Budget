...
const MeterReadings: React.FC<Props> = ({ readings, setReadings, settings }) => {
  ...
  const handleSave = () => {
    ...
    const prev = readings.filter(r => r.type === newReading.type).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
  ...
  const getIcon = (type: string) => {
    switch(type) {
      ...
      default: return <Droplets className={type === 'water_hot' ? 'text-red-400' : 'text-blue-400'} />;
    }
  };
             ...
             {readings.slice(0, 5).map(r => (
                <div key={r.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                   <div className="flex items-center gap-3">
                      {getIcon(r.type)}
                      <div>
                         <div className="font-bold text-sm">{getName(r.type)}</div>
                         <div className="text-[10px] text-gray-400 font-bold">{new Date(r.date).toLocaleDateString()}</div>
                      </div>
                   </div>
                   <div className="text-right">
                      <div className="font-black text-lg">{r.value}</div>
                      {r.prevValue && <div className="text-[10px] font-bold text-gray-400">+{r.value - r.prevValue}</div>}
                   </div>
                </div>
             ))}
             ...
             <motion.div initial={{scale:0.9}} animate={{scale:1}} exit={{scale:0.9}} className="relative bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl space-y-4">
                ...
                <div className="flex gap-2">
                   {['water_cold', 'water_hot', 'electricity'].map(t => (
                      <button key={t} onClick={() => setNewReading({...newReading, type: t as any})} className={`flex-1 py-3 rounded-xl border ${newReading.type === t ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
                         {getIcon(t)}
                      </button>
                   ))}
                </div>
                <input type="number" placeholder="Значение" value={newReading.value || ''} onChange={e => setNewReading({...newReading, value: Number(e.target.value)})} className="w-full bg-gray-50 p-4 rounded-xl font-bold text-sm outline-none" />
