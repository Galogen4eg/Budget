    ...
    <div className="fixed inset-0 z-[2000] flex items-end md:items-center justify-center">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 no-scrollbar">
            {MOCK_NOTIFICATIONS.length === 0 ? (
                ...
                MOCK_NOTIFICATIONS.map(notif => (
                    <div key={notif.id} className="bg-white dark:bg-[#1C1C1E] p-4 rounded-2xl shadow-sm border border-gray-50 dark:border-white/5 relative overflow-hidden">
                        <div className="flex justify-between items-start mb-1">
                            <h4 className="font-bold text-sm text-[#1C1C1E] dark:text-white">{notif.title}</h4>
                            <span className="text-[10px] font-bold text-gray-300 dark:text-gray-600">{notif.time}</span>
                        </div>
        ...
        <div className="p-6 bg-white dark:bg-[#1C1C1E] border-t border-gray-100 dark:border-white/5">
            <button onClick={onClose} className="w-full py-4 bg-gray-100 dark:bg-[#2C2C2E] text-gray-500 dark:text-white font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-gray-200 dark:hover:bg-[#3A3A3C] transition-colors">
                Закрыть
            </button>
        </div>
      </motion.div>
