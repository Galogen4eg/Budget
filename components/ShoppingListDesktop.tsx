...
const ShoppingListDesktop: React.FC<ShoppingListProps> = ({
  ...
  const { updateSettings, productCatalog, addProductToCatalog } = useData();
  ...
  const handleSaveItem = async (e: React.FormEvent) => {
    ...
    if (editingItem) {
        ...
        const existingItem = items.find(i => i.title.trim().toLowerCase() === inputValue.trim().toLowerCase() && !i.completed);
        
        if (existingItem) {
            // Add amount to existing item
            const newAmount = (parseFloat(existingItem.amount || '0') + amountValue).toString();
            const updatedItem = {
                ...existingItem,
                amount: newAmount,
                unit: existingItem.unit || unitValue as any,
            };
        ...
        <div className="space-y-8 mt-4">
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleSaveItem} className="space-y-4">
              <div className="relative">
                ...
                />
                {inputValue.length > 1 && productCatalog && productCatalog.filter(c => c.title.toLowerCase().includes(inputValue.toLowerCase()) && c.title.toLowerCase() !== inputValue.toLowerCase()).length > 0 && (
                    <div className="absolute top-full mt-2 w-full z-10 bg-white dark:bg-[#2C2C2E] rounded-2xl shadow-xl overflow-hidden max-h-40 overflow-y-auto">
                        {productCatalog.filter(c => c.title.toLowerCase().includes(inputValue.toLowerCase()) && c.title.toLowerCase() !== inputValue.toLowerCase()).slice(0, 5).map(suggestion => (
                            <button
                                type="button"
                                key={suggestion.id}
                                onClick={() => {
                                    setInputValue(suggestion.title);
                                    setSelectedCategoryId(suggestion.category || 'other');
                                    setUnitValue(suggestion.unit || 'шт');
                                }}
          ...
          <div className="space-y-2 pt-4 border-t border-black/5 dark:border-white/5">
            <button 
              ...
              className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-500/10 transition-all font-bold text-sm"
            >
              <BookOpen size={18} />
              Справочник товаров
            </button>
