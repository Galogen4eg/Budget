    ...
    <div className="space-y-4">
      <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2.5rem] shadow-soft dark:shadow-none border border-white dark:border-white/5">
        <div className="flex justify-between items-center mb-6">
           <h3 className="font-black text-lg text-[#1C1C1E] dark:text-white">Мои подписки</h3>
           <button onClick={() => { resetForm(); setIsModalOpen(true); }} className="w-10 h-10 bg-blue-50 dark:bg-blue-900/30 text-blue-500 rounded-xl flex items-center justify-center"><Plus size={20}/></button>
        </div>
        ...
        {subscriptions.length === 0 ? (
          ...
          <div className="space-y-3">
            {subscriptions.map(sub => {
              ...
              return (
                <div key={sub.id} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-[#2C2C2E] rounded-2xl border border-gray-100 dark:border-white/5">
                  <div className="flex items-center gap-4">
                     <div className="w-10 h-10 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 rounded-xl flex items-center justify-center font-bold text-lg">
                        {sub.name.charAt(0)}
                     </div>
                     <div>
                        <div className="font-bold text-[#1C1C1E] dark:text-white">{sub.name}</div>
                  ...
                  <div className="flex items-center gap-3">
                     <span className="font-black text-sm text-[#1C1C1E] dark:text-white">{sub.amount} {sub.currency}</span>
