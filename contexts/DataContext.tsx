...
import { 
  ...
  Project,
  LoyaltyCard, SavingsGoal, LearnedRule, Category, AppNotification, Reminder, AIKnowledgeItem, CatalogItem
} from '../types';
import { 
  FAMILY_MEMBERS, INITIAL_CATEGORIES, DEFAULT_RULES, DEMO_TRANSACTIONS,
  DEMO_SHOPPING_ITEMS, DEMO_EVENTS, DEMO_GOALS, DEMO_PROJECTS, DEMO_LOYALTY_CARDS, DEMO_MANDATORY_EXPENSES,
...
interface DataContextType {
  ...
  addProductToCatalog: (item: Omit<CatalogItem, 'id'> | CatalogItem) => Promise<void>;
  deleteProductFromCatalog: (id: string) => Promise<void>;
  settings: AppSettings;
  setSettings: React.Dispatch<React.SetStateAction<AppSettings>>;
  updateSettings: (newSettings: Partial<AppSettings>) => Promise<void>; // New unified method
  projects: Project[];
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateSettings = async (newSettings: Partial<AppSettings>) => {
      ...
      if (familyId && user) {
          await saveAppSettings(familyId, user.uid, mergedSettings as AppSettings);
      }
  };
  ...
  useEffect(() => {
    ...
    if (!familyId) {
        ...
        loadLocal('productCatalog', (data: CatalogItem[]) => setProductCatalog(data && data.length > 0 ? data : DEFAULT_PRODUCT_CATALOG), DEFAULT_PRODUCT_CATALOG);
    ...
    const unsubs = [
      ...
      subscribeToCollection(familyId, 'productCatalog', (data) => setProductCatalog(data.length > 0 ? data as CatalogItem[] : DEFAULT_PRODUCT_CATALOG)),
  ...
  useEffect(() => {
      ...
      if (!authLoading && !isInitialLoad.current && !familyId) {
          ...
          localStorage.setItem('local_productCatalog', JSON.stringify(productCatalog));
      ...
      authLoading, familyId, transactions, shoppingItems, events, 
      goals, projects, loyaltyCards, settings, members, reminders, 
      aiKnowledge, dismissedNotificationIds, localRules, productCatalog
  ]);
  ...
  const addProductToCatalog = async (item: Omit<CatalogItem, 'id'> | CatalogItem) => {
      const cleanTitle = item.title.trim();
      if (!cleanTitle) return;
      const existing = productCatalog.find(c => c.title.toLowerCase() === cleanTitle.toLowerCase());
      if (existing) return;

      const newItem: CatalogItem = {
          id: ('id' in item && item.id) ? item.id : Date.now().toString() + Math.random().toString(36).substr(2, 4),
          title: cleanTitle,
          category: item.category || 'other',
          unit: item.unit || 'шт'
      };

      const updated = [...productCatalog, newItem];
      setProductCatalog(updated);
      if (familyId) await addItem(familyId, 'productCatalog', newItem);
      await updateSettings({ productCatalog: updated });
  };

  const deleteProductFromCatalog = async (id: string) => {
      const updated = productCatalog.filter(c => c.id !== id);
      setProductCatalog(updated);
      if (familyId) await deleteItem(familyId, 'productCatalog', id);
      await updateSettings({ productCatalog: updated });
  };
  ...
  const value = {
    ...
    productCatalog, addProductToCatalog, deleteProductFromCatalog,
