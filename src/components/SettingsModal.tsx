...
import { 
  ...
  Wallet, Tag, ChevronDown, Sparkles, Globe,
  LayoutGrid, Shield, Lock, Key, Copy, 
  Users, Share, LogOut, ChevronRight, Calculator, 
  GripVertical, Loader2, Menu, 
  AppWindow, MoreHorizontal, ArrowLeft, 
  ArrowRight, Eye, EyeOff, ChevronLeft, Save, Calendar, Circle,
  ChevronUp, AlertOctagon, ShoppingBag, ShieldCheck, BellRing,
  FolderOpen, ArrowUp, ArrowDown, Zap, Gift, RefreshCw, Settings2, Moon, Sun, MessageSquareQuote, Send,
  Cloud, CloudOff, BrainCircuit, Mail, RefreshCcw, MoveUpRight, Play
} from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ settings, onClose, onUpdate, savingsRate, setSavingsRate, members, onUpdateMembers, categories, onUpdateCategories, onDeleteCategory, learnedRules, onUpdateRules, currentFamilyId, onJoinFamily, onLogout, transactions = [], onDeleteTransactionsByPeriod, onUpdateTransactions, onOpenDuplicates }) => {
  ...
  const handleSaveMember = async () => {
      ...
      if (editingMember) {
          ...
          if (currentFamilyId && email && email !== editingMember.email && !editingMember.userId) {
              try { await createInvitation(currentFamilyId, email, editingMember.id); } catch (e) {}
          }
      } else {
          const newMemberId = Math.random().toString(36).substr(2, 9);
          const newMember: FamilyMember = { id: newMemberId, name: tempMemberName, color: tempMemberColor, email: email || undefined, isAdmin: false, avatar: '', userId: '' };
          onUpdateMembers([...members, newMember]);
          if (currentFamilyId && email) {
              try { await createInvitation(currentFamilyId, email, newMemberId); } catch (e) {}
          }
      }
  ...
  const handleDeleteMember = async () => {
      ...
      if (members.length <= 1) { alert("Нельзя удалить последнего участника"); return; }
      if (confirm(`Удалить участника ${editingMember.name}?`)) {
          onUpdateMembers(members.filter(m => m.id !== editingMember.id));
          if (currentFamilyId) {
              try { await deleteItem(currentFamilyId, 'members', editingMember.id); } catch (e) {}
          }
          setIsMemberModalOpen(false);
      }
  };
  ...
  const handleUpdateFamilyId = async () => {
      ...
      if (!targetId || targetId === currentFamilyId) return;
    ...
    switch (activeSection) {
            ...
            <div className="space-y-6">
                <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] border border-gray-100 dark:border-white/10 shadow-sm">
                    ...
                    <div className="bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl mb-4 border border-gray-100 dark:border-white/5">
                        <input type="text" placeholder="Добавить факт (напр. код от домофона 123)" value={newFact} onChange={e => setNewFact(e.target.value)} onKeyPress={e => e.key === 'Enter' && newFact.trim() && (addAIKnowledge(newFact.trim()), setNewFact(''))} className="w-full bg-white dark:bg-[#1C1C1E] p-3 rounded-xl text-sm font-bold outline-none text-[#1C1C1E] dark:text-white mb-2" />
                        <button onClick={() => { if(newFact.trim()) { addAIKnowledge(newFact.trim()); setNewFact(''); } }} className="w-full py-2 bg-[#1C1C1E] dark:bg-white text-white dark:text-black font-black uppercase text-xs rounded-xl">Добавить в память</button>
                    </div>
                ...
                <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] space-y-6 border border-gray-100 dark:border-white/10 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl">
                            ...
                            <input type="number" step="0.01" placeholder="0" value={settings.initialBalance || ''} onChange={e => handleChange('initialBalance', e.target.value === '' ? 0 : parseFloat(e.target.value))} className="w-full bg-transparent font-bold text-lg outline-none text-[#1C1C1E] dark:text-white placeholder:text-gray-300" />
                        </div>
                        <div className="bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl">
                        ...
                        <div className="bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl">
                            ...
                            <input type="number" min="0" max="23" value={settings.dayStartHour ?? 8} onChange={e => handleChange('dayStartHour', parseInt(e.target.value))} className="w-full bg-transparent font-bold text-lg outline-none text-[#1C1C1E] dark:text-white" />
                        </div>
                        <div className="bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl">
                            <label className="text-xs font-bold text-gray-500 block mb-1">Конец дня (ч)</label>
                            <input type="number" min="1" max="24" value={settings.dayEndHour ?? 23} onChange={e => handleChange('dayEndHour', parseInt(e.target.value))} className="w-full bg-transparent font-bold text-lg outline-none text-[#1C1C1E] dark:text-white" />
                        </div>
                    </div>
                    <div>
        ...
        case 'members': return (
            <div className="space-y-6 relative h-full">
                <AnimatePresence>
                    {isMemberModalOpen && (
                        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="absolute inset-0 z-50 bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] border border-gray-100 dark:border-white/10 shadow-2xl flex flex-col">
                            ...
                            <div className="flex-1 space-y-6">
                                <div>
                                    ...
                                    <input type="text" value={tempMemberName} onChange={(e) => setTempMemberName(e.target.value)} placeholder="Имя" className="w-full bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl font-bold text-lg outline-none text-[#1C1C1E] dark:text-white border border-transparent focus:border-blue-500 transition-colors" autoFocus />
                                </div>
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="email" value={tempMemberEmail} onChange={(e) => setTempMemberEmail(e.target.value)} placeholder="user@gmail.com" className="w-full bg-gray-50 dark:bg-[#2C2C2E] p-4 pl-12 rounded-2xl font-medium text-sm outline-none text-[#1C1C1E] dark:text-white border border-transparent focus:border-blue-500 transition-all dark:bg-black/20" />
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-3 bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl">
                                        {PRESET_COLORS.map(c => (
                                            <button key={c} onClick={() => setTempMemberColor(c)} className={`w-10 h-10 rounded-full transition-transform ${tempMemberColor === c ? 'scale-110 ring-4 ring-offset-2 ring-blue-500 dark:ring-offset-[#2C2C2E]' : 'hover:scale-105'}`} style={{ backgroundColor: c }} />
                                        ))}
                                    </div>
                                </div>
                            </div>
                            <div className="flex gap-3 mt-auto">
                                {editingMember && <button onClick={handleDeleteMember} className="p-4 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-2xl hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"><Trash2 size={24} /></button>}
                                <button onClick={handleSaveMember} disabled={!tempMemberName.trim()} className="flex-1 bg-blue-500 text-white p-4 rounded-2xl font-black uppercase text-sm shadow-xl active:scale-95 transition-transform disabled:opacity-50">Сохранить</button>
                            </div>
                        </motion.div>
                    )}
                </AnimatePresence>
                <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] border border-gray-100 dark:border-white/10 shadow-sm h-full flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar space-y-3">
                        {members.map(member => (
                            <div key={member.id} onClick={() => handleOpenMember(member)} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-[#2C2C2E] rounded-2xl cursor-pointer hover:bg-gray-100 dark:hover:bg-[#3A3A3C] transition-colors">
                                <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleOpenMember()} className="w-full mt-4 bg-[#1C1C1E] dark:bg-white text-white dark:text-black py-4 rounded-2xl font-black uppercase text-xs flex items-center justify-center gap-2 active:scale-95 transition-transform"><Plus size={18} strokeWidth={3} /> Добавить участника</button>
                </div>
            </div>
        );
        case 'categories': return (
            <div className="h-full overflow-hidden">
                <CategoriesSettings categories={categories} onUpdateCategories={onUpdateCategories} onDeleteCategory={onDeleteCategory} learnedRules={learnedRules} onUpdateRules={onUpdateRules} settings={settings} transactions={transactions} onUpdateTransactions={onUpdateTransactions} />
            </div>
        );
                                ...
                                <div key={widget.id} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-[#2C2C2E] rounded-2xl border dark:border-white/5">
                                    <div className="flex items-center gap-2 flex-1">
                                        <div className="flex flex-col gap-1 mr-2">
                                            <button onClick={() => !isFirst && moveWidget(idx, 'up')} disabled={isFirst} className={`p-1 rounded-md transition-colors ${isFirst ? 'text-gray-300 dark:text-gray-600' : 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900/30'}`}><ArrowUp size={14} /></button>
                                            <button onClick={() => !isLast && moveWidget(idx, 'down')} disabled={isLast} className={`p-1 rounded-md transition-colors ${isLast ? 'text-gray-300 dark:text-gray-600' : 'text-blue-500 hover:bg-blue-100 dark:hover:bg-blue-900/30'}`}><ArrowDown size={14} /></button>
                                        </div>
            ...
            <div className="space-y-6">
                <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] border border-gray-100 dark:border-white/10 shadow-sm space-y-4">
                    ...
                    <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-[#2C2C2E] rounded-2xl border border-gray-100 dark:border-white/5">
                        <div className="flex items-center gap-3"><div className="text-gray-400"><Send size={20}/></div><div><div className="font-bold text-sm">Авто-отправка событий</div></div></div>
                    ...
                    <div className="pt-2 space-y-4 border-t dark:border-white/10">
                        <TemplateEditor label="Шаблон списка покупок" value={settings.shoppingTemplate || '🛒 *Список покупок*\n\n{items}'} onChange={(val) => handleChange('shoppingTemplate', val)} variables={['{items}', '{total}', '{date}']} previewData={{ '{items}': '• Молоко\n• Хлеб', '{total}': '2', '{date}': '10.10.2023' }} />
                        <TemplateEditor label="Шаблон событий" value={settings.eventTemplate || '📅 *{title}*\n🕒 {date} {time}\n📝 {description}\n👥 {members}\n📋 {checklist}'} onChange={(val) => handleChange('eventTemplate', val)} variables={['{title}', '{date}', '{time}', '{description}', '{members}', '{checklist}']} previewData={{ '{title}': 'Врач', '{date}': '10.10.2023', '{time}': '14:00', '{description}': 'Взять полис', '{members}': 'Мама, Папа', '{checklist}': '• Паспорт' }} />
                    </div>
                </div>
            </div>
        );
        case 'family': return (
            <div className="space-y-6">
                <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] border border-gray-100 dark:border-white/10 shadow-sm">
                    ...
                    <div className={`p-5 rounded-2xl flex items-center gap-4 mb-6 ${currentFamilyId ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 border border-green-100 dark:border-green-900/30' : 'bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-300 border border-orange-100'}`}>
                        <div className="p-3 rounded-full bg-white/50 dark:bg-white/10">{currentFamilyId ? <Cloud size={24} /> : <CloudOff size={24} />}</div>
                        <div><div className="font-black text-sm uppercase tracking-wide">{currentFamilyId ? 'Синхронизация активна' : 'Локальный режим'}</div><div className="text-xs mt-1 font-medium opacity-80">{auth.currentUser?.email ? <>Аккаунт: {auth.currentUser.email}</> : <>Вы вошли как Гость</>}</div></div>
                    </div>
                    <div className="bg-gray-50 dark:bg-[#2C2C2E] p-4 rounded-2xl mb-4 border border-gray-100 dark:border-white/5">
                        <div className="flex items-center gap-2 mb-3"><Key size={16} className="text-purple-500" /><p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Текущий Family ID</p></div>
                        <div className="flex flex-col gap-3">
                            <div className="flex gap-2">
                                <input type="text" value={newFamilyId} onChange={(e) => setNewFamilyId(e.target.value)} className={`flex-1 bg-white dark:bg-[#1C1C1E] p-4 rounded-xl font-mono text-sm font-bold border transition-all ${newFamilyId !== currentFamilyId ? 'border-blue-500' : 'border-transparent'}`} placeholder="Введите ID" />
                                {newFamilyId !== currentFamilyId && <button onClick={handleUpdateFamilyId} disabled={isJoining} className="bg-blue-500 text-white p-4 rounded-xl flex items-center justify-center active:scale-90 transition-transform disabled:opacity-50 shadow-lg">{isJoining ? <Loader2 size={20} className="animate-spin" /> : <RefreshCcw size={20} />}</button>}
                            </div>
                            {newFamilyId !== currentFamilyId && <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="px-1"><label className="flex items-center gap-3 p-3 bg-white dark:bg-[#1C1C1E] rounded-xl border border-blue-100 dark:border-blue-900/20 cursor-pointer"><div className={`w-10 h-10 rounded-lg flex items-center justify-center transition-colors ${shouldMigrate ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-[#3A3A3C] text-gray-400'}`}><MoveUpRight size={20} /></div><div className="flex-1 min-w-0"><p className="text-xs font-bold dark:text-white leading-tight">Перенести текущие данные</p></div><Switch checked={shouldMigrate} onChange={() => setShouldMigrate(!shouldMigrate)} /></label></motion.div>}
                        </div>
                    </div>
                    {currentFamilyId && <button onClick={() => { const link = `${window.location.origin}/?join=${currentFamilyId}`; if (navigator.share) navigator.share({ title: 'Семейный Бюджет', text: 'Присоединяйся к нашему бюджету!', url: link }); else { navigator.clipboard.writeText(link); alert("Ссылка скопирована"); } }} className="w-full mt-6 bg-[#1C1C1E] dark:bg-white text-white dark:text-black py-4 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95 transition-transform shadow-lg"><Share size={16} /> Поделиться кодом</button>}
                </div>
            </div>
        );
            ...
            <div className="space-y-6">
                <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] space-y-5 border border-gray-100 dark:border-white/10 shadow-sm">
                  ...
                  <div className={`p-5 rounded-[1.5rem] border transition-all ${isAIEnabled ? 'bg-purple-50 dark:bg-purple-900/10 border-purple-100 dark:border-purple-900/20' : 'bg-gray-50 dark:bg-[#2C2C2E] border-gray-100 dark:border-white/5'}`}>
                      <div className="flex items-center gap-3 mb-4">
                          <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${isAIEnabled ? 'bg-white dark:bg-white/10 text-purple-600 dark:text-purple-400' : 'bg-white dark:bg-white/5 text-gray-400'}`}><Sparkles size={20} /></div>
                          <div><h3 className={`font-bold text-sm ${isAIEnabled ? 'text-purple-900 dark:text-purple-100' : 'text-gray-700 dark:text-gray-300'}`}>AI Ассистент (Gemini)</h3></div>
                      </div>
                      <div className="space-y-3">
                          <div className="relative"><Key size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" /><input type="password" value={settings.geminiApiKey || ''} onChange={e => handleChange('geminiApiKey', e.target.value)} className="w-full bg-white dark:bg-[#1C1C1E] pl-11 pr-4 py-3 rounded-xl font-mono text-xs outline-none border border-transparent focus:border-purple-500 transition-all text-[#1C1C1E] dark:text-white shadow-sm" placeholder="Вставьте API ключ (AIza...)" /></div>
                          {settings.geminiApiKey && <button onClick={handleTestKey} disabled={aiTestStatus === 'loading'} className={`w-full py-2.5 rounded-xl text-[10px] font-black uppercase flex items-center justify-center gap-2 transition-all ${aiTestStatus === 'success' ? 'bg-green-500 text-white' : aiTestStatus === 'error' ? 'bg-red-500 text-white' : 'bg-white dark:bg-[#2C2C2E] border border-gray-200 dark:border-white/5'}`}>{aiTestStatus === 'loading' ? <Loader2 size={14} className="animate-spin"/> : aiTestStatus === 'success' ? <Check size={14}/> : <Play size={14}/>}{aiTestStatus === 'success' ? 'Работает!' : aiTestStatus === 'error' ? 'Ошибка' : 'Проверить'}</button>}
                      </div>
                  </div>
  ...
  return (
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-0 md:p-4">
        ...
        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative bg-[#F2F2F7] dark:bg-black w-full max-w-7xl h-full md:h-[85vh] md:rounded-[3rem] rounded-none shadow-2xl overflow-hidden flex flex-col md:flex-row border dark:border-white/10">
            ...
            <div className={`flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-[#F2F2F7] dark:bg-black pt-safe md:pt-0 ${showMobileMenu ? 'hidden' : 'flex'}`}>
                ...
                <div className={`flex-1 overflow-y-auto no-scrollbar overscroll-contain ${activeSection === 'categories' ? 'p-0' : 'p-4 md:p-8 pb-24 md:pb-8'}`}>
                    <div className={`max-w-4xl mx-auto w-full h-full ${activeSection === 'categories' ? 'max-w-full' : ''}`}>{renderSectionContent()}</div>
                </div>
            </div>
        </motion.div>
    </div>
