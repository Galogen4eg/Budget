...
const ShoppingListMobile: React.FC<ShoppingListProps> = ({ 
  ...
  const handleSaveItem = async () => {
      ...
      if (editingItem) {
          ...
          // Check for duplicate (active or completed)
          const existingItem = items.find(i => i.title.toLowerCase() === cleanTitle.toLowerCase());

          if (existingItem) {
              // Merge logic: Add amount if both are numbers, otherwise just overwrite with new + updated flag
              const currentAmount = parseFloat(existingItem.amount || '0') || 0;
              const addedAmount = parseFloat(newItemAmount) || 1; // Default to 1 if no amount
              const newAmount = currentAmount + addedAmount;

              const updatedItem: ShoppingItem = {
                  ...existingItem,
                  amount: String(newAmount), // Sum amounts
                  unit: newItemUnit, // Prefer new unit
                  completed: false, // Bring back to active list
                  category: selectedAisle !== 'other' ? selectedAisle : existingItem.category
              };
  ...
  const processVoiceInputWithGemini = async (text: string) => {
      ...
      try {
          ...
          if (Array.isArray(parsedItems) && parsedItems.length > 0) {
              ...
              // Note: For voice batch, we simply append. 
              // A more advanced version would also deduplicate here, but batch adding is faster.
