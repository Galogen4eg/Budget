...
const SmartDebtPlanner: React.FC<Props> = ({ transactions, debts, settings }) => {
  ...
  const incomePattern = useMemo((): IncomePattern | null => {
    ...
    // 1. Try strict matching first (Category or Keywords)
    let salaryTxs = incomeTxs.filter(t => 
        t.category === 'salary' || 
        (t.note || '').toLowerCase().includes('зарплата') || 
        (t.note || '').toLowerCase().includes('salary') ||
        (t.note || '').toLowerCase().includes('аванс')
    );

    // 2. Fallback: Heuristic for large regular deposits if strict match fails (> 10k)
    // This handles cases where user forgot to categorize income
    if (salaryTxs.length === 0) {
        salaryTxs = incomeTxs.filter(t => t.amount >= 10000);
    }

    if (salaryTxs.length === 0) return null;

    // Sort by date descending
    const sorted = [...salaryTxs].sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    
    // Take up to 6 most recent for average
    const recent = sorted.slice(0, 6);
    
    const totalAmount = recent.reduce((sum, t) => sum + t.amount, 0);
    const avgAmount = totalAmount / recent.length;
    
    const days = recent.map(t => new Date(t.date).getDate());
    const avgDay = Math.round(days.reduce((a, b) => a + b, 0) / days.length);

    return {
        amount: Math.floor(avgAmount),
        dayOfMonth: avgDay || 1, // Fallback to 1st if something goes wrong
                ...
                <div className="bg-orange-50 dark:bg-orange-900/10 p-4 rounded-2xl border border-orange-100 dark:border-orange-900/20">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-500 mt-1">Добавьте транзакцию с категорией "Зарплата" (или >10000р) или укажите вручную.</p>
                ...
                <div className="bg-indigo-50/50 dark:bg-indigo-900/10 p-4 rounded-2xl border border-indigo-100 dark:border-indigo-900/20 flex justify-between items-center">
                    <div>
                        <div className="text-[10px] font-black uppercase text-indigo-400 tracking-wider">
                            Ваш доход ({incomePattern.isManual ? 'Вручную' : `Анализ: ${incomePattern.sourceCount} шт.`})
                        </div>
